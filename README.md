...
Your browser opens to `http://localhost:8050` with a cyberpunk-styled dashboard. Every CLI flag is exposed in the UI, with explanations, tooltips, a guide, and live log streaming. Save your settings, click Start, talk.

See [`web_ui/README.md`](./web_ui/README.md) for the full guide and [`web_ui/Guide.md`](./web_ui/Guide.md) for the in-app walkthrough.
...
This installs the package in editable mode and makes the `speech-to-speech` and `speech-to-speech-web` console scripts available.

For the dashboard wrapper:
