...
from speech_to_speech.arguments_classes.chatterbox_tts_arguments import ChatterboxTTSHandlerArguments
...
class ParsedArguments:
    ...
    parakeet_onnx_stt_handler_kwargs: ParakeetOnnxSTTHandlerArguments
...
def parse_arguments() -> ParsedArguments:
    ...
    return ParsedArguments(
        ...
        parakeet_onnx_stt_handler_kwargs=by_type[ParakeetOnnxSTTHandlerArguments],
...
def prepare_all_args(
    ...
    rename_args(parakeet_onnx_stt_handler_kwargs, "parakeet_onnx")
    ...
    rename_args(chatterbox_tts_handler_kwargs, "chatterbox")
...
def _build_pipeline_handlers(
    *,
    ...
    stt = get_stt_handler(
        ...
        mlx_audio_whisper_stt_handler_kwargs,
        parakeet_tdt_stt_handler_kwargs,
        parakeet_onnx_stt_handler_kwargs,
    )
...
def _build_realtime_pipeline_unit(
    *,
    ...
    parakeet_onnx_kw = deepcopy(parakeet_onnx_stt_handler_kwargs)
    ...
    chatterbox_tts_kw = deepcopy(chatterbox_tts_handler_kwargs)
    ...
    handlers = _build_pipeline_handlers(
        ...
        parakeet_onnx_stt_handler_kwargs=parakeet_onnx_kw,
        ...
        chatterbox_tts_handler_kwargs=chatterbox_tts_kw,
        speculative_turns=speculative_turns,
    )
    for h in handlers:
    ...
    if module_kwargs.mode == "local":
        ...
        pool = [
            _build_realtime_pipeline_unit(
                ...
                parakeet_onnx_stt_handler_kwargs=parakeet_onnx_stt_handler_kwargs,
                ...
                chatterbox_tts_handler_kwargs=chatterbox_tts_handler_kwargs,
            )
...
def get_stt_handler(
    ...
    if module_kwargs.stt == "whisper":
    ...
    elif module_kwargs.stt == "parakeet-onnx":
        from speech_to_speech.STT.parakeet_onnx_handler import ParakeetOnnxSTTHandler

        # Per-backend --parakeet-onnx-device wins; fall back to the global
        # --device flag (if the user set it explicitly), then "auto".
        per_backend_device = parakeet_onnx_stt_handler_kwargs.parakeet_onnx_device
        setup_kwargs = {
            **vars(parakeet_onnx_stt_handler_kwargs),
            "device": per_backend_device or module_kwargs.device or "auto",
        }
        ...
        raise ValueError(
            ...
            "parakeet-tdt, parakeet-onnx, or paraformer."
        )
...
def get_tts_handler(
    ...
    elif module_kwargs.tts == "chatterbox":
        try:
            from speech_to_speech.TTS.chatterbox_tts_handler import ChatterboxTTSHandler
        except ImportError as e:
            raise ImportError(
                'Chatterbox is optional. Install it with `pip install "speech-to-speech[chatterbox]"`.'
            ) from e

        return ChatterboxTTSHandler(
            stop_event,
            queue_in=lm_response_queue,
            queue_out=send_audio_chunks_queue,
            setup_args=(should_listen,),
            setup_kwargs=vars(chatterbox_tts_handler_kwargs),
        )
    else:
        raise ValueError("The TTS should be either chatTTS, facebookMMS, pocket, kokoro, qwen3, or chatterbox")
...
def main() -> None:
    ...
    prepare_all_args(
        ...
        args.parakeet_onnx_stt_handler_kwargs,
