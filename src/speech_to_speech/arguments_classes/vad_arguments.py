...
class VADHandlerArguments:
    ...
    speculative_reopen_ms: int = field(
        default=1000,
        metadata={
            "help": "In realtime mode, keep a soft-ended turn reopenable for this many milliseconds unless a response commits it. Default is 1000 ms."
        },
    )
    ...
    smart_turn: bool = field(
        default=False,
        metadata={
            "help": "In realtime mode, use Smart Turn v3.2 after Silero finalizes a segment to choose how long assistant output remains speculative. Disabled by default to preserve existing VAD behavior; pass --smart_turn to enable it."
        },
    )
